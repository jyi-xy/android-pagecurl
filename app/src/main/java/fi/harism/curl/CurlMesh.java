...
public class CurlMesh {
	...
	private static final String TAG = CurlMesh.class.getName();
	...
	// Flag for translate rect. the translate point is cur position.
	private static final boolean DRAW_TRANSLATE_RECT = false;
	...
	private FloatBuffer mBufTransRectangle;
	...
	private int mTransRectPointCount;
	...
	public CurlMesh(int maxCurlSplits) {
		...
		if (DRAW_TRANSLATE_RECT) {
			mTransRectPointCount = 6;
			ByteBuffer hvbb = ByteBuffer
					.allocateDirect(mTransRectPointCount * 2 * 4);
			hvbb.order(ByteOrder.nativeOrder());
			mBufTransRectangle = hvbb.asFloatBuffer();
			mBufTransRectangle.position(0);
		}
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		/*记垂直于cur.x, cur.y的线为La，求平行于La,且与未卷起部分的四边形相交的顶点，
		卷起部分分成mMaxCurlSplits份，均平行La与四边形相交的交点。*/
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (DRAW_TRANSLATE_RECT) {
				...
				if (i == 2) j = 3;
				if (i == 3) j = 2;
				mBufTransRectangle.put(2 * j, (float) v.mPosX);
				mBufTransRectangle.put(2 * j + 1, (float) v.mPosY);
			}
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			if (intersections.size() == 2) {
				...
				if (DRAW_TRANSLATE_RECT) {
					if (scanXmin == 0.0f) {
						mBufTransRectangle.put(8, (float) v1.mPosX);
						mBufTransRectangle.put(9, (float) v1.mPosY);
						mBufTransRectangle.put(10, (float) v2.mPosX);
						mBufTransRectangle.put(11, (float) v2.mPosY);
					}
				}
			...
			while (mArrOutputVertices.size() > 0) {
				...
				// Vertex lies within 'curl'.
					...
					Log.d(TAG, "beta:" + rotY / Math.PI * 180);
		...
		Log.d(TAG, "front:" + mVerticesCountFront + ", back:" +
				mVerticesCountBack + ", vertices:" + mBufVertices.position() / 3);
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		if (DRAW_TRANSLATE_RECT) {
			...
			gl.glVertexPointer(2, GL10.GL_FLOAT, 0, mBufTransRectangle);
			gl.glDrawArrays(GL10.GL_LINES, 0, mTransRectPointCount);
			gl.glDisable(GL10.GL_BLEND);
		}
